...
|`.reviewedBy(Reviewer)` / `.reviewedBy(String script)`
...
a|`gradle approvejFindLeftovers` +
`mvn approvej:find-leftovers`
|List leftover approved files

a|`gradle approvejCleanup` +
`mvn approvej:cleanup`
|Remove leftover approved files

a|`gradle approvejApproveAll` +
`mvn approvej:approve-all`
|Approve all unapproved files

a|`gradle approvejReviewUnapproved` +
