...
- the `id` is a UUID that's being generated randomly, and
- the `published` is a `LocalDateTime` set to now.
...
In the flowing example, the two dynamic properties are scrubbed with the built-in link:{javadoc-url}/core/org/approvej/scrub/UuidScrubber.html[`UuidScrubber`] and link:{javadoc-url}/core/org/approvej/scrub/InstantScrubber.html[`InstantScrubber`].
...
----
<1> so it makes sense to change the filename extension, so your IDE will apply appropriate syntax highlighting
