...
In order to change the way objects are being transformed to strings, you can use the link:{javadoc-url}/core/org/approvej/ApprovalBuilder.html[`ApprovalBuilder`]'s `printedBy` method and provide a link:{javadoc-url}/core/org/approvej/print/Printer.html[`Printer`]/`Function<T, String>` instance as an argument.
...
You can use this printer by calling the `printedBy` method on the builder.
...
`defaultPrintFormat`:: the default print format class if none is specified via the `printedAs` method (see <<printing>>)
...
NOTE:: The former property `defaultPrinter` is deprecated and using it will cause an exception!
