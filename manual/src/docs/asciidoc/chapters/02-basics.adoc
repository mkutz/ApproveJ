...
=== Generic Multi Line Printer

ApproveJ provides a generic link:{javadoc-url}/core/org/approvej/print/MultiLinePrinter.html[`MultiLinePrinter`] that will print the object with each of its properties on a new line to make comparing the result easier.
...
----
<1> applies the `MultiLinePrinter` and returns a new `ApprovalBuilder<String>`
