...
[id=yaml_jackson_dependencies]
== Dependencies

[id=yaml_jackson_dependencies_jackson_2]
=== Jackson 2.x
...
[id=yaml_jackson_dependencies_jackson_3]
=== Jackson 3.x
...
[id=yaml_jackson_dependencies_imports]
=== Import Differences
...
The link:{javadoc-url}/yaml-jackson/org/approvej/yaml/jackson/YamlPrintFormat.html[`YamlPrintFormat`] allows you to print any object in YAML format.

To use YAML as the default print format for all approvals, <<default_print_format,configure it globally>>:
