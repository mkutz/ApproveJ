...
ifndef::backend-pdf[]
TIP: A link:pdf/approvej-cheat-sheet.pdf[printable PDF version] of this cheat sheet is available.
endif::[]


[id=cheat_sheet_entry_point]
== Entry Point
...
|`.reviewedBy(Reviewer fileReviewer)`
...
|`MarkdownTablePrintFormat.markdownTable()`
|Print as Markdown table (requires `database-jdbc`)

|`SqlPrintFormat.sql()`
|Pretty-print SQL with line breaks (requires `database-jdbc`)
|===


[id=cheat_sheet_scrubbers]
== Scrubbers

[id=cheat_sheet_scrubbers_string]
=== String Scrubbers
...
[id=cheat_sheet_scrubbers_date_time]
=== Date/Time Scrubbers
...
[id=cheat_sheet_scrubbers_field]
=== Field Scrubbers
...
[id=cheat_sheet_scrubbers_database]
=== Database Scrubbers
...
|`DatabaseScrubbers.columnValue(String columnName)`
|Scrub all values in a column by name (case-insensitive)
|===


[id=cheat_sheet_replacements]
== Replacements
...
|Mask characters (`A` → `A`, `a` → `a`, `0` → `1`)
|===


[id=cheat_sheet_approvers]
...
|`Reviewers.ai(String command)`
|Use an AI CLI tool to review and auto-approve changes
|===


[id=cheat_sheet_build_plugin]
== Build Plugin Tasks
...
|`reviewerScript`
|`APPROVEJ_REVIEWER_SCRIPT`
|_(none, required when reviewer is `script`)_

|`reviewerAiCommand`
|`APPROVEJ_REVIEWER_AI_COMMAND`
|_(none, required when reviewer is `ai`)_
...
|`APPROVEJ_DEFAULT_INLINE_VALUE_REVIEWER`
|`none`
|===
