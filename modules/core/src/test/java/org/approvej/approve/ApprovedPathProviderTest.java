...
class ApprovedPathProviderTest {
  ...
  void paths_no_extension_no_infix() {
    Path approvedPathNoExtensionNoInfix = Path.of("/path/to/some_file.txt");
  ...
  void paths_no_extension() {
    Path approvedPathNoExtension = Path.of("/path/to/some_file-approved");
    ...
    assertThat(pathProvider.receivedPath()).isEqualTo(Path.of("/path/to/some_file-received"));
  }
}
